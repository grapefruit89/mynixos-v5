...
[F-074] modules\apps\service-media-sonarr.nix | 4,34 KB | Tags: [SONARR, MEDIA, Aviation, types, config]
...
[F-061] modules\apps\service-media-jellyfin.nix | 3,75 KB | Tags: [JELLYFIN, config, MEDIA, Aviation, srePaths]
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      ...
      systemd.services.home-assistant = {
        description = "Home Assistant Core (Aviation-Grade)";
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      ...
      systemd.services.n8n = {
        description = "n8n Workflow Engine (Aviation-Grade)";
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      ...
      systemd.services.seerr = {
        description = "Seerr Media Request Service (Aviation-Grade)";
...
let
  ...
  nms = {
    ...
    description = "Music downloader with Titanium Sandboxing and ABC-Tiering.";
    layer = 40;
    nixpkgs.category = "services/media";
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      systemd.services.lidarr = {
        description = "Lidarr (Aviation-Grade)";
...
let
  ...
  nms = {
    ...
    description = "Indexer manager for *arr apps with Titanium Sandboxing.";
    layer = 40;
    nixpkgs.category = "services/media";
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      systemd.services.prowlarr = {
        description = "Prowlarr (Aviation-Grade)";
        after = [ "network.target" "postgresql.service" ];
        wantedBy = [ "multi-user.target" ];
...
let
  ...
  nms = {
    ...
    description = "Movie downloader with Titanium Sandboxing and ABC-Tiering.";
    layer = 40;
    nixpkgs.category = "services/media";
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      systemd.services.radarr = {
        description = "Radarr (Aviation-Grade)";
        after = [ "network.target" "postgresql.service" ];
        wantedBy = [ "multi-user.target" ];
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    systemd.services.sonarr-setup = {
      description = "Sonarr API Configuration (Aviation-Grade)";
...
let
  ...
  nms = {
    ...
    description = "TV series downloader with Titanium Sandboxing and ABC-Tiering.";
    layer = 40;
    nixpkgs.category = "services/media";
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      systemd.services.sonarr = {
        description = "Sonarr (Aviation-Grade)";
        after = [ "network.target" "postgresql.service" ];
        wantedBy = [ "multi-user.target" ];
...
let
  ...
  serviceBase = myLib.mkService { 
    ...
    port = nativePort; 
    useSSO = true; 
    description = "${name} Service (Aviation-Grade)"; 
  ...
  config = lib.mkIf (cfg.enable or false) {
    ...
    systemd.services.auto-locale-sync = {
      ...
      script = ''
        ...
        logger -t auto-locale "Aviation-Grade Sync: System localized to $COUNTRY"
      '';
    };
...
in {
  ...
  config = lib.mkIf (config.my.services.kernelSlim.enable) {
    ...
    systemd.services.kernel-slim-info = {
      ...
      script = ''
        logger -t kernel-slim "Aviation-Grade Hardened Kernel loaded"
        MODULES=$(lsmod | wc -l)
  ...
  config.assertions = [
    {
      ...
      message = " NIXHOME HORIZONTAL VIOLATION: Subdirectories in modules/ silos are strictly forbidden! (Aviation-Grade Rule)";
    }
  ];
}
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
...

    {
      ...
      services.mosquitto = {
        ...
        listeners = [{
          port = cfg.mqttPort;
          address = "127.0.0.1"; # Aviation-Grade: Only local access
      ...
      services.zigbee2mqtt = {
        ...
        settings = {
          ...
          advanced = {
            ...
            pan_id = 0x1a2b; # Aviation-Grade: Custom PAN-ID (Source: Fragment 18968)
          };
        };
      };
...
let
  ...
  securityChecks = [
    ...
    }
    {
      ...
      msg = " [SEC-NET-002]: NFTables is disabled! Using legacy iptables is not Aviation-Grade.";
    }
    {
    ...
    }
    {
      ...
      msg = " [SEC-COR-001]: Titanium Hardened Core module is missing or disabled!";
    }
    {
