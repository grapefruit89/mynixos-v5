...
let
 ...
 FREE_A=$(${pkgs.coreutils}/bin/df --output=avail "${srePaths.tierA}" | tail -1)
 ...
 FREE_B=$(${pkgs.coreutils}/bin/df --output=avail "${srePaths.tierB}" | tail -1)
 ...
 for dev in "''${PHYSICAL_HDDS[@]}"; do
     if ${pkgs.hdparm}/bin/hdparm -C "$dev" | grep -q "active/idle"; then
         IS_AWAKE=$((IS_AWAKE + 1))
     fi
 done
 ...
 if [ "$DO_MOVE" -eq 1 ]; then
     ...
     while [ "$FREE_GB_B" -lt "$TARGET_FREE_GB" ] && [ "$COUNT" -lt "$MAX_ITERATIONS" ]; do
         ...
         if [ "$DRY_RUN" -eq 1 ]; then
             ...
             mkdir -p -- "$DEST_DIR"
             if ${pkgs.rsync}/bin/rsync -a -- "$OLDEST" "$TARGET_DIR/$REL_PATH"; then
                 rm -f -- "$OLDEST"
                 echo "🚚 Archived eldest media file: $REL_PATH"
             fi
             FREE_SPACE_B=$(${pkgs.coreutils}/bin/df --output=avail "$SOURCE_DIR" | tail -1)
