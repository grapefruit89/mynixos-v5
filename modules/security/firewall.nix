      ...
      ruleset = ''
        table inet filter {
          ...
          chain input {
            ...
            # 5. Global SSH Protection
            ip saddr @allowed_countries tcp dport ${toString config.my.ports.ssh} accept
    ...
    systemd.services.update-geoip-sets = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "update-geoip" ''
          ...
          # We generate a full transaction file to ensure that if any step fails,
          # the firewall remains in its previous state.
          ...
          if nft -f "$NFT_CMD"; then
            ...
            echo "❌ Failed to apply atomic update. Keeping old rules."
            exit 1
          fi
        '';
      };
